...
class MyQueue:
    ...
    def put(self, item: QueueMessage) -> None:
        """
        ...
        """
        self._queue.put(obj = item, block = False, timeout = 0)
...
class ServerProcess(multiprocessing.Process):
    ...
    def __init__(self, logger: Any, name: str, url: str, events: ServerEvents, queue_server : MyQueue, queue_ir : MyQueue, queue_main : MyQueue) -> None:
        ...
        @self.sio.event
        def reset_alarm(data = None) -> None:
            self.logger.warning("reset alarm")
            self.queue_ir.put(("reset_alarm", data or {}))
            
        @self.sio.event
        def reset_error(data = None) -> None:
            self.logger.warning("reset Error")
            self.queue_ir.put(("reset_error", data or {}))
            
        @self.sio.event
        def set_config(data) -> None:
            self.logger.warning("set config")
            self.queue_ir.put(("set_config", data))
            
        
        @self.sio.event
        def set_temperature(data) -> None:
            self.logger.warning(f"set tempreture {data}")
            self.queue_ir.put(("set_temperature", data))
            
        @self.sio.event
        def manual_start_record(data = None) -> None:
            self.logger.warning("manual start record")
            self.queue_ir.put(("manual_start_record", data or {}))
            
        
        @self.sio.event
        def manual_stop_record(data = None) -> None:
            self.logger.warning("manual stop record")
            self.queue_ir.put(("manual_stop_record", data or {}))
        
        @self.sio.event
        def manual_call_record(data = None) -> None:
            self.logger.warning("manual call record")
            self.queue_ir.put(("manual_call_record", data or {}))
            
        @self.sio.event
        def call_live_tempreture(data) ->None:
            self.logger.warning("call live tempreture")
            self.queue_ir.put(("call_live_tempreture", data or {}))
            
        @self.sio.event
        def call_history_tempreture(data = None) -> None:
            self.logger.warning("call history tempreture")
            self.queue_ir.put(("call_history_tempreture", data or {}))
    ...
    def run(self) -> None:
        ...
        while not self.events.shutdown.is_set():
            try:
                # Check if something is sent to the server via queue
                command, payload = self.queue_server.get(timeout = 0.01)
                self.logger.debug(f"Forwarding to server: {command} | {payload}")
                self.sio.emit(event=command, data=payload)
            except Empty:
                pass  # no queue message – ignore
...
class IRAppEvents:
    shutdown: MyEvent = field(default_factory=MyEvent)
    heartbeat: MyEvent = field(default_factory=MyEvent)
    error_from_ir_process: MyEvent = field(default_factory=MyEvent)
    ir_process_okay: MyEvent = field(default_factory=MyEvent)


class IRAppProcess(multiprocessing.Process):
    def __init__(self, logger, name: str, events: IRAppEvents, queue_ir: MyQueue):
        super().__init__(name=name)
        self.logger = logger
        self.events = events
        self.queue_ir = queue_ir
    ...
    def run(self):
        self.logger.debug("IRAppProcess started")
        try:
            import main  # import here to avoid circular dependency
            main.queue_ir = self.queue_ir  # Inject the queue into main.py
            main.events_ir = self.events  # Inject the heartbeat events into main.py
            main.main()
        except Exception as e:
            self.logger.critical(f"IRAppProcess crashed: {e}", exc_info=True)
            self.events.error_from_ir_process.set()
        finally:
            self.logger.debug("IRAppProcess exited")
