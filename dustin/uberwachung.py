...
from my_processes import MyQueue, IRAppProcess, IRAppEvents
USE_MOCK_RELAIS = True

if USE_MOCK_RELAIS:
    from mock_relais import Relay
else:
...
def main ():
    ...
    ProcessManager.processes["Server"] = server_process
    # Create logger and events for IR app
    logger_ir_app = my_logger.get_logger("ir_app")
    events_ir = IRAppEvents()

    # Start IR app process
    ir_app_process = IRAppProcess(logger=logger_ir_app, name="IRApp", events=events_ir, queue_ir=queue_ir)
    # Register it in the process manager
    ProcessManager.processes["IRApp"] = ir_app_process
